...
func _ready() -> void:
	...
	%lb_master.text = "%.02f" % AudioServer.get_bus_volume_db(0)
	%btn_sfx_mute.button_pressed = AudioServer.is_bus_mute(1)
	%sl_sfx.value = AudioServer.get_bus_volume_linear(1)
	%sfx_test.stop()
	%lb_sfx.text = "%.02f" % AudioServer.get_bus_volume_db(1)
	%btn_music_mute.button_pressed = AudioServer.is_bus_mute(2)
	%sl_music.value = AudioServer.get_bus_volume_linear(2)
	%lb_music.text = "%.02f" % AudioServer.get_bus_volume_db(2)
	%btn_ui_mute.button_pressed = AudioServer.is_bus_mute(3)
	%sl_ui.value = AudioServer.get_bus_volume_linear(3)
	%ui_test.stop()
	%lb_ui.text = "%.02f" % AudioServer.get_bus_volume_db(3)
	%btn_voice_mute.button_pressed = AudioServer.is_bus_mute(4)
	%sl_voice.value = AudioServer.get_bus_volume_linear(4)
	%voice_test.stop()
	%lb_voice.text = "%.02f" % AudioServer.get_bus_volume_db(4)
...
func _on_btn_voice_mute_toggled(toggled_on: bool) -> void: AudioServer.set_bus_mute(4, toggled_on)
...
func _on_sl_ui_value_changed(value: float) -> void:
	...
	%ui_test.play()
	%lb_ui.text = "%.02f" % AudioServer.get_bus_volume_db(3)
func _on_sl_voice_value_changed(value: float) -> void:
	AudioServer.set_bus_volume_linear(4, value)
	if not %voice_test.playing: %voice_test.play()
	%lb_voice.text = "%.02f" % AudioServer.get_bus_volume_db(4)
